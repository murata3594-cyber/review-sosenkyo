...
def load(path: Path, errors: list[str]) -> dict:
    try: return json.loads(path.read_text(encoding="utf-8"))
        ...
        errors.append(f"Invalid JSON {path.relative_to(ROOT)}: {exc}"); return {}
...
def audit_research(path: Path, errors: list[str], warnings: list[str]) -> None:
    ...
    raw_flag=policy.get("raw_review_text_stored")
    if raw_flag is True:
        errors.append(f"{path.relative_to(ROOT)}: raw review storage is forbidden")
    elif raw_flag is None:
        warnings.append(f"{path.relative_to(ROOT)}: legacy ledger missing raw_review_text_stored=false flag")
    researched=str(data.get("researched_at", ""))
    if researched and not valid_date(researched): errors.append(f"{path.relative_to(ROOT)}: invalid researched_at {researched}")
...
def main() -> int:
    errors=[]; warnings=[]
    manifest=load(MANIFEST, errors); affiliate=load(AFFILIATE, errors)
    catalog_articles=set(affiliate.get("articles", {})); published=manifest.get("published", [])
    if len(published) < 10: warnings.append(f"Published article count is only {len(published)}")
    fake_firsthand=("実際に使ってみた","実際に使用してみた","私が使った","私が試した","使ってみました")
    for item in published:
        article_name=item.get("article", ""); research_name=item.get("research", "")
        if article_name not in catalog_articles: errors.append(f"Affiliate catalog missing published article: {article_name}")
        article=ROOT/article_name
        if article.exists():
            source=article.read_text(encoding="utf-8"); parser=TextParser(); parser.feed(source); body="".join(parser.text)
            if parser.h1 != 1: errors.append(f"{article_name}: expected exactly one h1, found {parser.h1}")
            if len(body) < 1200: warnings.append(f"{article_name}: visible text is short ({len(body)} chars)")
            if len(parser.external) < 2: warnings.append(f"{article_name}: fewer than 2 external source links")
            if 'id="sources"' not in source: warnings.append(f"{article_name}: sources section id missing")
            if any(phrase in body for phrase in fake_firsthand): errors.append(f"{article_name}: possible unsupported firsthand claim")
            if len(parser.meta_description) < 45: warnings.append(f"{article_name}: meta description may be too short")
        if research_name:
