    ...
    if research:
        ...
        if path.exists():
            try:
                ...
                for key in ("checked_at", "checkedAt", "updated_at", "updated"):
                    value = data.get(key)
...
def schema_for(filename: str, title: str, description: str, url: str, cfg: dict, manifest: dict) -> dict:
    ...
    return {
        ...
        "url": url,
        "inLanguage": "ja-JP",
        "isPartOf": {"@type": "WebSite", "name": cfg.get("site_name", "レビュー総選挙"), "url": canonical_url(str(cfg["production_url"]).rstrip("/"), "index.html")},
    }
